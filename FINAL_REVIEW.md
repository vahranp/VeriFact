...
- **A genuine table-column-misalignment risk, caught by evidence re-grounding rather than hidden.**
  ...
  "header semantics not modeled" limitation already documented for the Delhivery corpus (see
  `INTERVIEW_NOTES.md`) — now independently reproduced on an RBI table with a different structure
